...
@section('scripts')
  ...
  <script type="application/javascript" async defer crossorigin="anonymous"
      ...
      <div class="card-body">
        <div class="row no-gutters">
          <div class="col-md-8">
            ...
            @if($event->is_stem == 1)
              <div class="is_stem">
                <p><span class="badge badge-info">STEM/STEAM Event</span></p>
              </div>
            @endif
            <br>
          ...
          <div class="col-md-4">
            ...
            <div>
              ...
              @elseif ($event->location->name == "Online")
                ...
                @else
                  <iframe src="https://www.youtube.com/embed/?listType=user_uploads&list=DroidbuildersUK" width="250"
                    height="200"></iframe>
                @endif
              @else
                <div class="map-responsive">
                  <iframe width="200" height="200" frameborder="0" style="border:1"
              ...
              <br>
              <span class="float-right">
                <a class="btn-sm btn-link"
                  href="{{ route('location.show', $event->location->id)}}">{{ $event->location->name}}</a>
        ...
        @can('Edit Events')
          <a class="btn btn-edit" style="width:auto;"
            href="{{ route('admin.events.edit', $event->id) }}">{{ __('Edit') }}</a>
          <a class="btn btn-edit" style="width:auto;"
            href="{{ route('admin.events.addimage', ['event_id' => $event->id]) }}">{{ __('Add Image') }}</a>
          ...
          <button type="button" class="btn btn-primary m-2" data-toggle="modal"
            data-target="#addContactModal">{{ __('Add Contact') }}</button>

        @endcan
        <div>
          <span class="float-right">
            <small>{{ __('Event Submitted By') }}: {{  $event->organiser->forename }}
              {{  $event->organiser->surname }}</small>
          </span>
        </div>
      </div>
    </div>
      ...
      <div class="card-body">
          ...
          <ul>
            @foreach($event->attended as $user)
              <li>
                ...
                @endif
                @can('Edit Events')
                  <a href="{{ route('admin.events.attendance.deny', ['event_id' => $event->id, 'user_id' => $user->id])}}"><i
        ...
        <ul>
          @foreach($event->notgoing as $user)
            <li><a href="{{ route('user.show', $user->id) }}">{{ $user->forename ?? "Deactivated"}}
                {{ $user->surname ?? "User"}}</a></li>
          @endforeach
        </ul>
        @endcan
        @can('Edit Events')
          <span id="export" class="btn btn-primary" data-href={{ route('admin.events.export', $event->id) }}
            onclick="exportList(event.target);">Download list as CSV
          </span>
        @endcan
        @endif
      </div>
