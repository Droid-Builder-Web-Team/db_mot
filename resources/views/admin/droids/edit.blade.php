...
@section('content')
    <div class="card">
                ...
                <div class="col-lg-12 margin-tb">
                    <div class="pull-right pb-3">
                        <a class="btn btn-mot-invert" style="width:auto; color:white;"
                            href="{{ route('droid.show', $droid->id) }}">Back</a>
                    </div>
                    <div class="pull-left">
                        <h2>Edit Droid</h2>
                    </div>
        ...
        <div class="card-body">
            {!! html()->model($droid)->form('PUT', route('admin.droids.update', $droid->id))->open() !!}
            @if ($errors->any())
                    ...
                    <div class="form-group">
                        ...
                        <select class="js-example-basic-single" name=club_id>
                            @foreach ($clubs as $club)
                                <option value="{{ $club->id }}" @if ($droid->club_id == $club->id) selected @endif>
                                    {{ $club->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-xs-3 col-sm-3 col-md-3">
                    <div class="form-group">
                        ...
                        <select class="form-control form-control-light" name="radio_controlled">
                            ...
                            <option value="Yes" <?php echo $droid->radio_controlled == 'Yes' ? 'selected' : ''; ?>>Yes
                            </option>
                        </select>
                    </div>
                </div>
            ...
            <div class="form-row">
                ...
                @if ($droid->club->hasOption('tier_two'))
                    {!! html()->hidden('tier_two', 'No') !!}
                    <div class="col-md-1 mb-3">
                        <label>Tier 2?</label>
                        <input type="checkbox" name="tier_two" {{ $droid->tier_two == 'Yes' ? 'checked="Yes"' : 'value=Yes' }}
                            class="form-control">
                    </div>
                @endif
            </div>
